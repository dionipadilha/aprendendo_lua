...
local function rangeInputHandler(start, stop, step)
  ...
  local inputErrorMsgs = {
    "Start value must be a number",
    "Stop value must be a number or nil",
    "Step value must be a number or nil"
  }
  assert(type(start) == "number", inputErrorMsgs[1])
  assert(type(stop) == "number" or stop == nil, inputErrorMsgs[2])
  assert(type(step) == "number" or step == nil, inputErrorMsgs[3])
...
local function createRangeIterator(start, stop, step)
  local i = start - step
  return function()
    i = i + step
...
local function range(start, stop, step)
  start, stop, step = rangeInputHandler(start, stop, step)
  local rangeIterator = createRangeIterator(start, stop, step)
